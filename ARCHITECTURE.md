...
`marginTensionYield`, `marginShearUlt`, `marginInteraction`, `marginSeparation`,
...
stiffness geometry is missing. `engine.marginTensionYield` consults the SAME
gate and implements the yield-side analogue (5020B Eq. 16/17) when not
assured. ✅ Phase 3.2 adds
...
thread-strength checks, using the PITCH-DIAMETER method — thread-shear area
...
76-77 / 79 basis, with the 0.75·π pitch-diameter area substituted on both
the external and internal sides; 5020B prints no thread-shear equations)
against the design
...
`NominalTorque`/`TorqueTolerance` (no `ThermalRate` column — LATER CHANGE,
see below). The joint
...
template's first row was originally the DABJ §9 class-problem joint
expressed in the schema (later repointed to real catalog hardware in a
DABJ-Section-9-like configuration once `validation.dabjSection9` started
building that fixture's geometry in code instead — see `tests/tBulk.m`'s
header) and the settings template carries the §9 temperatures + factors,
checked by `tests/tBulkParsers.m`. LATER CHANGE: the template's
`ThermalRate` column (and the GUI's supplied-rate checkbox/field) were
removed — `ThermalRate` is not an analyst-facing input; it is set only by
validation fixtures (`PreloadSpec.ThermalRate`, see `model.PreloadSpec`),
so every parsed/GUI-built joint's thermal preload now comes from the
CTE-mismatch/joint-stiffness path (TM-106943 Eq. 10) instead.
✅ Step 2b adds the workbook generator:
...
`b = lib.bolt("#10-32 UNF"); m = lib.material("A286");`. The **Headless
...
callers degrade rather than stop:
...
So the deferral costs **accuracy, not capability**: those joints analyse, just
pessimistically. Closing it is worth real margin on sizing studies, where 20–30%
conservatism can push a design up a diameter — but it is not a bound on what can
be analysed, and should not be described as one.

**Two lessons, and the second is the sharper one.** First: before marking
anything "Phase N later", check its fan-in here. Second: fan-in alone is not the
answer — read the callers' fallback paths too. This graph is generated from call
edges and cannot see a `catch` that substitutes a conservative default, which is
exactly what makes the `stiffness` deferral tolerable.
...
- **Validate against a known-good published answer key** — the numbers are validated
  against the **DABJ course book §9 public worked example** (primary), with a second
  wave of acceptance cases later (Phase 3.4). A feature list is not an answer key:
...
- **The pitch-diameter thread-shear method** (✅ 3.3) — thread stripping uses
...
- **`engine.systemTensileAllowable` stays public, not `+engine/private/`** — it
  ...
  buy nothing: every consumer (`engine.marginTensionUlt`, and now
  `engine.boltSizingSweep`'s optional threaded-member-context path) already
...
- **Validation answer key (✅ Phase 2.3):** `validation.dabjSection9()` encodes the
  **DABJ §9 worked example** — the full Joint/LoadCase/Factors built from the
  ...
  this case — and later second-wave acceptance cases (Phase 3.4) — and assert a numeric
...
| Separation + bolt-yield margins (`engine.marginSeparation`, `engine.marginTensionYield`) | 2.6 | ✅ |
...
| Stiffness wiring: thermal preload (TM-106943 Eq. 10) + tension rupture branch (5020B Eq. 10) + bolt-yield rupture branch (5020B Eq. 16/17) | 3.1b | ✅ |
| Member checks: bearing (TM-106943 Eq. 72-74, validated vs DABJ Ex 5-b) + shear tear-out (Eq. 69-71) + bearing-under-head (Eq. 75) | 3.2 | ✅ |
| Thread checks: bolt-thread shear + nut strength + tapped-hole parent thread (0.75·π·E·Le pitch-diameter method; tapped-parent cross-checked vs DABJ Ex 6-a) + insert via Heli-Coil rated pull-out — all 15 checks implemented | 3.3 | ✅ |
| Second validation wave (additional acceptance cases) | 3.4 | ⏳ |
