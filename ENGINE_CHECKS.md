...
| 2 | Tension-Yield | **5020B Eq. 15** (separation first) or **Eq. 16/17** (yield first) — same Fig. 8 gate as row 1; **Eq. 18** for the bolt's allowable when unrated | Fastening-system minimum — bolt yield and the nut / insert / tapped parent's `As·Fsy`, §4.4.2 (the allowable p30 names when it introduces Eq. 17) | `FF·FSY·PtL`; Eq. 17 adds preload and `n·φ` | No yield mode of the system assessable at all; or the yield-first branch needed and stiffness unavailable |
...
| 4 | Interaction | **5020B Eq. 20** (body, 2.5 / 1.5) · **Eq. 22** (threads, 1.2 / 2.0), bending `Rb = fbu/Ftu` inside the tension bracket | Bolt's own ultimate — *not* the system minimum | Ratio at design loads | Bolt ultimate allowable unavailable; a supplied moment with no section or no `Ftu`; or `Joint.ShearTransferCondition = ClearanceOrGapped` with **no** moment supplied (§4.4.4 bending required — supply `LoadCase.BoltBendingLimitMoment`) |
...
| | |
|---|---|
| **Bolt bending from shear** | A supplied limit moment (`LoadCase.BoltBendingLimitMoment`) is always used: `fbu = 32·Mbu/(π·d³)` on the shear-plane section, added inside the Eq. 20/22 tension bracket as `Rb = fbu/Ftu`. What the tool does not do is *derive* a moment from the shear load. With no moment supplied, `Joint.ShearTransferCondition` records which §4.4.4 case applies: `NotDeclared` (default) computes `fbu = 0` with the exemption marked ASSUMED; `CloseToleranceOrInterference` the same, marked VERIFIED; `ClearanceOrGapped` reports the Interaction row as **NotEvaluated** (`R = NaN`) instead of a silently non-conservative number. Eq. 21/23 (plastic bending, `fbu/Fbu`) are not implemented |
| **Exact per-layer stiffness for mixed-modulus stacks** | A stack whose flanges differ in modulus computes via a thickness-weighted harmonic-mean member modulus (`Ebar = tFit/sum(t_i/E_i)`, NASA TM-106943 Eq. 34) fed into the same frustum, not true per-layer slicing. Exact when material boundaries coincide with the frustum knee (the ordinary two-plate joint); up to +23% high on `kc` / −14% low on `phi` — unconservative for bolt tension — for stacks with soft layers at both bearing faces. Inserts and tapped holes compute the same way via the shortened grip `L = t1 + D/2`; a threaded-in joint MISSING frustum geometry still falls back to a conservative `phi = 1` |
