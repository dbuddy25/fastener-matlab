...
`stiffness` refuses no CONFIGURATION. Insert and tapped-hole joints compute
via the shortened grip `L = t1 + D/2`; a mixed-modulus flange stack computes
via the thickness-weighted harmonic-mean member modulus `Ebar` (NASA TM-106943
Eq. 34 — `TOOL_DIFFERENCES.md` §7.5 has the approximation's measured error
bound). `stiffness` errors only on missing DATA (e.g. no `HeadBearingDiameter`, no
