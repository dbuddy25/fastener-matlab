...
Design spec for the GUI layer (`+gui`).
...
2. **One package, `+gui`**, launched by `gui.launch` (or `fastenerTool`).
...
flag); `gui.ReferencesView` renders it; `gui.referencesFolder` remembers
...
A single `gui.AppState < handle` owns everything: `Project`, `Joint`,
...
**Page contract** — abstract base `gui.Page`:
...
All color goes through `gui.palette`. No literal RGB triple appears anywhere
else in `+gui`. Carry the semantics forward from `gui.palette`: muted gray =
