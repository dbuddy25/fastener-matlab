...
> This is the *pre*-compile pass, in MATLAB. "Check first on the packaged app",
> at the end, is the *post*-compile pass, on the `.exe`. Different lists on
...
`+data/library` folder added as files required for the app. Nothing else needs
adding by hand: the user guide is generated by `report.userGuide` on first open.
Done when the exe runs on a clean Windows box with only the MATLAB Runtime.

### Check first on the packaged app

These are the places source and deployed genuinely differ:

- [ ] **The library loads.** A "hardware library failed" alert means the
      `+data/library` folder did not make it into the bundle.
- [ ] **Help → About** shows a version (`toolVersion` resolved).
- [ ] **Help → User Guide** builds the PDF and opens it — exercises Report
      Generator and `prefdir()` as a writable location inside the deployed app.
- [ ] **Materials & Hardware → add a custom entry → Save → restart.** The entry
      survives (`data.Library.userPath` falls back to `prefdir()` because the
      install directory is not writable).
- [ ] **Run a bulk workbook** — `data.makeTemplate` and `runWorkbook` resolve the
      library the same way the GUI does.

**What "validated" can mean for the packaged app:** the DABJ §9 published example
plus the automated suite. Checks verified only against non-public data cannot be
re-run from this repository, so decide what "validated packaged release" means
before stamping 1.0.0.
