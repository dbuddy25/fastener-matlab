...
%                                    OPTIONAL in the file: when the
%                                    joint_name column is absent or the
%                                    cell is blank, JointName is "" —
%                                    meaning "the caller supplies the
%                                    element -> joint mapping" (the GUI's
%                                    Element Mapping tab). A real FEM force
%                                    export carries no joint names, so ""
%                                    is a legitimate value, not an error.
%                                    Headless runs (engine.runBulk /
%                                    runWorkbook) have no mapping step, so
%                                    there a blank JointName produces a
%                                    per-row Error in engine.analyzeBulk.
...
%                                    (default false)
%
%   [el, info] = data.loadElements(...) also reports the rows that were
%   NOT parsed, so a skipped row can never vanish silently:
%       info.SkippedRowCount (1,1) double — rows that had content but no
%                                    element_id (skipped, since an element
%                                    row is keyed by its id)
%       info.SkippedRows     (1,:) double — their row numbers in the grid
%                                    as read (row 1 = first row of the
%                                    sheet/CSV, header included). Fully
%                                    blank rows (spreadsheet padding) are
%                                    ignored quietly and NOT counted.
%   The single-output call form is unchanged — omit info to get exactly
...
%       reversible (opt)
%
%   Rows with a blank element_id are skipped and reported via `info`
%   (fully blank padding rows are ignored quietly). joint_name is OPTIONAL
%   — a blank or absent joint_name keeps the row with JointName = "".
...
el = struct("ElementId", {}, "JointName", {}, "LoadCaseName", {}, ...
            ...
            "Reversible", {});
skippedRows = zeros(1, 0);
for r = hdrRow+1:size(raw, 1)
    id    = getText(raw, names, r, "element_id", "");
    joint = getText(raw, names, r, "joint_name", "");   % optional: "" = caller maps
    if strlength(id) == 0
        % Not an element row (rows are keyed by element_id). Fully blank
        % rows are spreadsheet padding — ignored quietly; a row that HAS
        % content but no element_id is reported through `info` so it
        % cannot vanish silently.
        if ~rowIsBlank(raw, r)
            skippedRows(end+1) = r; %#ok<AGROW>
        end
        continue
    end
...
info = struct("SkippedRowCount", numel(skippedRows), ...
...
%   Blankness matches getval's notion: missing, blank/whitespace text, or
%   a scalar NaN. Anything else (a number, TRUE/FALSE, non-blank text)
%   counts as content.
tf = true;
for c = 1:size(raw, 2)
    v = raw{r, c};
