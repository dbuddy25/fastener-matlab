...
%   is a sample four-bolt SHCS/nut joint built from real catalog hardware,
%   NOT the DABJ Section 9 class-problem joint; see data.makeTemplate and
%   validation.dabjSection9). Optional columns may be
...
%                               blank -> 0)
%       (No ThermalRate column: it is not an analyst-facing input.
%       PreloadSpec.ThermalRate stays 0 for every parsed joint, so the
%       thermal preload change always comes from CTE mismatch + joint
%       stiffness, TM-106943 Eq. 10 — see model.PreloadSpec.)
%
...
%       HelicoilLengthRatio  -> (Insert) ThreadedMember.EngagementRatio
%                               (stored as the ratio itself, NOT multiplied
%                               out here -- engine/private/resolveEngagementLength
%                               computes Le = EngagementRatio x Bolt.NominalDiameter
%                               at analysis time, so a case file keeps the
...
%       marginInsert derives the parent-material area itself from
%       StiPitchDiameter below when this field is NaN, which it always is
%       for a bulk-loaded row.)
%       (No StiPitchDiameter column: it is a catalogue lookup, not analyst
%       input. For an Insert row, ThreadedMember.StiPitchDiameter is
%       auto-resolved from lib.insertFor(Bolt's NominalDiameter, TPI)'s
%       StiPitchDiameterMin — NaN when the catalogue has no entry for that
%       thread size, e.g. #0-80/#5-44, which no insert is manufactured for.)
%
...
cols = ["Name", "Bolt", "BoltMaterial", "BoltSpec", ...
        ...
        "TorqueTolerance", "BodyLengthInGrip", ...
...
switch tmType
    ...
    case model.ThreadedMemberType.Insert
        ...
        if strlength(k) > 0, tm.Material = lib.material(k); end
        % Stored as the ratio itself (ThreadedMember.EngagementRatio), NOT
        % multiplied out here: engine/private/resolveEngagementLength
        % resolves Le = EngagementRatio x Bolt.NominalDiameter at analysis
        % time (see model.ThreadedMember's header). Multiplying here would
        % discard the analyst's actual "xD" intent and bake in an inch
        % value tied to whichever bolt this row happened to name.
        v = getNum(raw, names, r, "HelicoilLengthRatio", NaN);
        if ~isnan(v), tm.EngagementRatio = v; end
        % No HelicoilShearArea column -- ThreadedMember.ShearEngagementArea
        % stays at the model default (NaN); engine.marginInsert derives the
        % area itself from StiPitchDiameter below (see this function's
        % header).
        tm.RatedUltimateLoad = getNum(raw, names, r, "HelicoilRatedLoad", 0);
        % StiPitchDiameter — catalogue-derived (NASM33537 insert table),
        % never an analyst-typed column: resolved by the row's OWN bolt
        % thread size via lib.insertFor, exactly like BoltSpec's
        % auto-lookup above. insertFor returns [] when no catalogue entry
        % matches (e.g. #0-80/#5-44, no insert manufactured that size) ->
        % NaN, the model's "unknown" sentinel; never fall back to the
        % bolt's own PitchDiameter.
        ins = lib.insertFor(b.NominalDiameter, b.ThreadsPerInch);
        if ~isempty(ins)
            tm.StiPitchDiameter = ins.StiPitchDiameterMin;
        end
    otherwise
...
% No ThermalRate column: PreloadSpec.ThermalRate stays its model default
% (0), so every parsed joint takes the CTE-mismatch/stiffness thermal
% path (TM-106943 Eq. 10) — see model.PreloadSpec.
