...
% the NaN.
iaRow = entry("Interaction", NaN, ia.Method, ia.Detail, ia.R, ia.Inputs);
if isnan(ia.R)
...
margins = [ ...
    entry("Tension-Ultimate", tu.MS, tu.Method, tu.Decision, NaN, tu.Inputs), ...
    entry("Tension-Yield",    ty.MS, ty.Method, ty.Detail, NaN, ty.Inputs), ...
    entry("Shear-Ultimate",   su.MS, su.Method, su.Detail, NaN, su.Inputs), ...
    iaRow, ...
    entry("Separation",       sp.MS, sp.Method, "", NaN, sp.Inputs), ...
    entry("Slip",             sl.MS, sl.Method, "", NaN, sl.Inputs), ...
    sbr, ...
    ...
    entry("Nut strength",              ns.MS, ns.Method, ns.Detail, NaN, ns.Inputs), ...
    entry("Insert internal-thread",    ii.MS, ii.Method, ii.Detail), ...
    entry("Insert external-thread",    ix.MS, ix.Method, ix.Detail, NaN, ix.Inputs), ...
...
r = engine.Result( ...
    ...
    Gate           = tu.Gate, ...
    Allowables     = tu.SystemAllowable, ...
    Bending        = ia.Bending, ...
    Warnings       = warnings);
end

% ---- Local helpers --------------------------------------------------------
function e = entry(name, ms, method, detail, r, inputs)
...
%
%   inputs (optional, default engine.eqInput() — the EMPTY 1x0 array): the
%   numbers substituted into this row's equation, as data. A row that has
%   not been wired for it yet, and every NotEvaluated row, carries the
%   empty array; a consumer renders "no inputs recorded" by testing
%   isempty(), never by looking for a sentinel value. Rows are wired one
%   margin function at a time (Separation and Nut strength first), so the
%   field being empty means "not surfaced here", NOT "this check consumed
%   no numbers".
if nargin < 5
    r = NaN;
end
