...
%       InsertExternal, Separation, Slip, SepBeforeRupture,
%       TappedParent                          14 margin MS values, pulled
%                                             from Result.Margins by Name
%                                             (NaN = NotEvaluated)
%       InteractionR                          the NASA-STD-5020B Eq. 20-23
%                                             interaction RATIO (Pass iff
%                                             R <= 1 -- the OPPOSITE
%                                             direction from every MS
%                                             column above), pulled from
%                                             Result.Margins("Interaction").R
%                                             -- NOT .MS, which is NaN by
%                                             design for this row (see
%                                             engine.analyze's INTERACTION
%                                             IS NOT A MARGIN note).
%                                             Column is named InteractionR
%                                             (not "Interaction") precisely
%                                             so nothing downstream can
%                                             mistake it for an ordinary
%                                             margin and apply the wrong
%                                             sign test or min()-envelope
%                                             direction to it.
%       WorstMargin, GoverningCheck           from the Result (InteractionR
%                                             never enters this pick)
...
%       Warnings                              TRAILING column (placed after
%                                             every column above, Note
%                                             included) — "" when
%                                             Result.Warnings is empty,
%                                             else each row's Severity +
%                                             Message joined "; " (worst
%                                             first: bolt length, then
%                                             preload — engine.analyze's
%                                             own order). Placed LAST
%                                             deliberately: the GUI Bulk
%                                             grid discovers margin columns
%                                             POSITIONALLY, so a trailing
%                                             column is invisible to that
%                                             discovery and needs no GUI
%                                             change. An errored row (Error
%                                             set) carries "" here, same as
%                                             Note.
%
...
%   the batch still runs. A BLANK JointName (joint_name is optional in
%   data.loadElements — blank means the caller supplies the mapping, as
%   the GUI's Element Mapping tab does) gets a specific Error telling the
%   headless user to add a joint_name column or map the elements in the
%   GUI. A DUPLICATE JointName (data.loadJointLibrary enforces no
%   uniqueness on Name, so a library can legally carry two rows with the
%   same Name) also gets a specific Error naming the ambiguity, rather
%   than silently resolving to whichever duplicate happens to be found
%   first — a duplicated joint row must be visible to the analyst, not
%   guessed.
%
...
%
%   Note on Interaction: NASA-STD-5020B Eq. 20-23 is a pass/fail CRITERION
%   (R <= 1), not a margin equation, so engine.marginInteraction reports a
%   ratio R (and Pass), not an MS — engine.analyze's Interaction Margins row
%   is therefore MS = NaN BY DESIGN (same reasoning as SepBeforeRupture
%   above; also excluded from WorstMargin/GoverningCheck), with its real
%   Pass/Fail carried in Status and its R value on the row's own R field
%   (and in Detail). This function's InteractionR column is sourced
%   DIRECTLY from that R field (Result.Margins("Interaction").R), NOT from
%   the generic per-column .MS pull the other 14 columns use — so it
%   carries the real ratio on every row where the check evaluates, never
%   NaN-by-construction. Consumers of this table (the GUI Bulk grid in
%   particular) must apply the R <= 1 criterion to this column, NOT the
%   MS >= 0 sign test the other 14 columns use, and must NOT fold it into
%   a min()-style envelope the way a true margin's worst case is taken
%   (larger R is worse, the opposite of smaller MS).
%
...
% Right: compact writetable-friendly column names. "Interaction" maps to
% "InteractionR" (not "Interaction") -- a deliberate rename flagging that
% this ONE column, alone among the 15, is sourced from .R rather than .MS
% and carries the opposite-direction R <= 1 criterion (see below).
...
errMsg    = strings(n, 1);
note      = strings(n, 1);
warn      = strings(n, 1);   % trailing Warnings column (see header note)
...
for k = 1:n
    ...
    %    joint_name is OPTIONAL in data.loadElements: a blank JointName
    %    means "the caller supplies the element -> joint mapping" (the
    %    GUI's Element Mapping tab overrides JointName before calling the
    %    engine). Reaching here with a blank name means no mapping was
    %    supplied — say that plainly instead of `Joint "" not found`.
    if strlength(el.JointName) == 0
        errMsg(k) = "Element """ + el.ElementId + """ has no joint_name and " + ...
            "no mapping was supplied — add a joint_name column to the " + ...
            "elements file, or map the elements to joints in the GUI's " + ...
            "Element Mapping tab.";
        continue
    end
    matches = find(libNames == el.JointName);
    if isempty(matches)
        errMsg(k) = "Joint """ + el.JointName + """ not found in the joint library.";
        continue
    end
    if numel(matches) > 1
        % data.loadJointLibrary enforces no Name uniqueness, so more than
        % one row can share a Name. Silently taking the FIRST match would
        % analyze this element against whichever duplicate happens to
        % come first — reported here instead of guessed, since nothing
        % about the ambiguity would otherwise show up in Error or Note.
        errMsg(k) = "Joint """ + el.JointName + """ is ambiguous: " + ...
            string(sprintf("%d", numel(matches))) + ...
            " rows in the joint library share this name — rename the " + ...
            "duplicate joints (or otherwise make Name unique) before analyzing.";
        continue
    end
    joint = jointLibrary(matches).Joint;
    ...
    try
        ...
        % 4) Run the full 15-check solver and pull each column by margin
        %    Name. Every column reads .MS EXCEPT "Interaction" -> the
        %    InteractionR column, which reads .R directly (NASA-STD-5020B
        %    Eq. 20-23 is a pass/fail criterion, not a margin; .MS is NaN
        %    by design on that row -- see engine.analyze's INTERACTION IS
        %    NOT A MARGIN note and this file's header).
        r = engine.analyze(joint, lc, factors);
        resNames = [r.Margins.Name];
        for c = 1:numel(marginNames)
            m = find(resNames == marginNames(c), 1);
            if ~isempty(m)
                if marginNames(c) == "Interaction"
                    ms(k, c) = r.Margins(m).R;
                else
        ...
        % 4b) Trailing Warnings column -- "" when clean, else each row's
        % Severity + Message joined "; " in engine.analyze's own order
        % (bolt length, then preload).
        if isempty(r.Warnings)
            warn(k) = "";
        else
            parts = arrayfun(@(w) w.Severity + ": " + w.Message, r.Warnings);
            warn(k) = strjoin(parts, "; ");
        end
    catch err
