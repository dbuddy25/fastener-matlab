...
%   the design bolt load the thread checks (bolt-thread shear,
...
%   gate is actually assured would be non-conservative — why this branch
...
%   assessable at all — rare, since the bolt-tension mode falls back to a
%   derived allowable, boltTensileAllowable, when unrated — see
%   separationBeforeRuptureGate), the CLAMPED form is used and Note says
%   the gate was not assessed rather than pretending a branch was chosen
%   on evidence.
%
%   phi handling (the stiffness call is wrapped, never allowed to crash;
%   phi is resolved the SAME way regardless of which branch's formula ends
%   up using it, so "not evaluated when joint geometry is unknown" stays a
%   single, consistent rule for every thread check):
...
%       computes this frustum form (shortened grip L = t1 + D/2), so a
%       fully-defined insert or tapped-hole joint gets a REAL phi. phi = 1
%       survives only as a FALLBACK for a threaded-in joint whose frustum
%       geometry is incomplete (no FlangeStack, no HeadBearingDiameter,
%       ...), where it remains a CONSERVATIVE bound: phi = kb/(kb+kc) <= 1
%       always, and Pb is monotonic increasing in phi, so assuming 1 can
%       only overstate Pb and understate the thread margins. Note names
%       the assumption and the reason for it. The fallback is DELIBERATELY
%       NOT extended to nut joints: those have always reported
%       NotEvaluated on missing geometry, and widening a bounding
%       assumption to cover them would hide real data gaps.
...
try
    ...
    if joint.ThreadedMember.Type ~= model.ThreadedMemberType.Nut
        % Threaded-in joint whose frustum geometry is incomplete — a
        % missing-DATA fallback. phi = 1 is a conservative BOUND, not an
        % estimate: phi = kb/(kb+kc) <= 1 always, and
        % Pb = PpMax + FF*FS*n*phi*PtL rises with phi, so assuming 1 can
        % only overstate Pb and understate the thread margins. Supplying
        % the frustum geometry (FlangeStack, HeadBearingDiameter,
        % BodyLengthInGrip) replaces it with the real phi and RELAXES
        % these margins.
        phi     = 1;
