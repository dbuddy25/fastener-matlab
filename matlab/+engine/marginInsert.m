...
r = struct("MS", MS, "Method", methodArea, "Detail", detail + ".", ...
    ...
    "Inputs", [areaInputs(joint, As, areaSrc), ...
        engine.eqInput("Fsu", Fsu, "psi", ...
            "ThreadedMember.Material.Fsu - the PARENT material the insert " + ...
            "is installed in, NOT the insert's own"), ...
        engine.eqInput("Fsy", sy.Fsy, "psi", ...
            "engine.shearYieldStrength - " + sy.Basis), ...
        engine.eqInput("Pult", allowUlt, "lbf", ...
            "NASA-STD-5020B 4.4.1 - Pult = As*Fsu"), ...
        engine.eqInput("Pyld", allowYld, "lbf", ...
            "this tool's yield CRITERION (4.4.2 prints no pull-out " + ...
            "equation) - Pyld = As*Fsy"), ...
...
terms = engine.eqInput("As", As, "in^2", areaSrc);
...
terms = [ ...
    engine.eqInput("D2", joint.ThreadedMember.StiPitchDiameter, "in", ...
        "ThreadedMember.StiPitchDiameter - the STI TAPPED-HOLE pitch " + ...
        "diameter (NASM33537 Rev 4 Table IV), at which the PARENT thread " + ...
        "shears; larger than the bolt's own pitch diameter"), ...
    engine.eqInput("Le", resolveEngagementLength(joint).Le, "in", ...
        "engine.private.resolveEngagementLength"), ...
    engine.eqInput("p", p, "in", ...
        "1/Bolt.ThreadsPerInch"), ...
    engine.eqInput("Le-1.125p", resolveEngagementLength(joint).Le - 1.125 * p, "in", ...
        "DERIVED CONVENTION, no equation number - NASM33537 11.1 installs " + ...
        "the insert top edge 0.75p-1.5p below the surface (midpoint " + ...
        "1.125p), so that much of the parent thread carries no pull-out"), ...
    terms];
end
