...
%MARGINSLIP  Slip (friction) margin of safety (NASA-STD-5020A Eq. 84 / Eq. 86).
...
%   (loadCase.BoltShear/TensileLimitLoad), per NASA-STD-5020A Eq. 86
...
%   distribution makes those differ), per NASA-STD-5020A Eq. 84:
...
if mode == model.SlipMode.Disabled
    r = struct( ...
        "MS",     NaN, ...
        "Method", "NASA-STD-5020A Eq. 84/86 (slip) — disabled");
    return
end
...
if mu == 0
    if mode == model.SlipMode.Joint
        label = "NASA-STD-5020A Eq. 84 (joint slip)";
    else
        label = "NASA-STD-5020A Eq. 86 (single-fastener slip)";
    end
    r = struct( ...
        "MS",     NaN, ...
...
if mode == model.SlipMode.Joint
    ...
    if isnan(PsLjoint) || isnan(PtLjoint)
        error("engine:marginSlip:jointLoadsRequired", ...
            "Joint slip (NASA-STD-5020A Eq. 84) requires joint-level limit loads " + ...
    ...
    nf = joint.BoltCount;
    % NASA-STD-5020A Eq. 84 (numerator) — Capacity = nf·μ·PpMin (friction resistance from clamp-up, all nf bolts)
    Capacity = nf * mu * preload.PpMin;
    % NASA-STD-5020A Eq. 84 (denominator) — Demand = FSslip·FFslip·(PsL_joint + μ·PtL_joint)
    ...
    % NASA-STD-5020A Eq. 84 — MS = (nf·μ·PpMin) / (FSslip·FFslip·(PsL_joint + μ·PtL_joint)) - 1
    MS = Capacity / Demand - 1;

    r = struct( ...
        "MS",     MS, ...
        "Method", sprintf("NASA-STD-5020A Eq. 84 (joint slip, %g bolts)", nf));
    ...
    if isnan(PsL) || isnan(PtL)
        error("engine:marginSlip:boltLoadsRequired", ...
            "Single-fastener slip (NASA-STD-5020A Eq. 86) requires per-bolt limit " + ...
            "loads (LoadCase.BoltShearLimitLoad / BoltTensileLimitLoad) — set " + ...
            "them on the LoadCase.");
    end

    % NASA-STD-5020A Eq. 86 (numerator) — Capacity = μ·PpMin (one fastener's friction resistance from clamp-up)
    Capacity = mu * preload.PpMin;
    % NASA-STD-5020A Eq. 86 (denominator) — Demand = FSslip·FFslip·(PsL + μ·PtL)
    % (this fastener's applied shear + friction lost to its applied tension)
    Demand = FSslip * FFslip * (PsL + mu * PtL);
    % NASA-STD-5020A Eq. 86 — MS = (μ·PpMin) / (FSslip·FFslip·(PsL + μ·PtL)) - 1
    MS = Capacity / Demand - 1;

    r = struct( ...
        "MS",     MS, ...
        "Method", "NASA-STD-5020A Eq. 86 (single-fastener slip)");
end
end
