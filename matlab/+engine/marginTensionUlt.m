...
%       Inputs                  struct array (engine.eqInput): the numbers
%                               substituted into WHICHEVER equation ran.
%                               The two branches take different terms and
%                               the array says which — Eq. 6 has only
%                               Ptu_allow and Ptu; Eq. 7 additionally has
%                               PpMax, n, phi and P'tu, because the bolt
%                               there carries the preload plus n·phi of the
%                               load. EMPTY on both not-evaluated exits.
%                               Ptu_allow is ONE term (the §4.4.1 system
%                               minimum); which mode set it is in
%                               Result.Allowables, not restated here.
%
...
if assured
    ...
    inputs = [ ...
        engine.eqInput("Ptu_allow", PtuAllow, "lbf", ...
            "engine.systemTensileAllowable (5020B 4.4.1 system minimum) - " + ...
            "governing mode " + sys.GoverningMode), ...
        engine.eqInput("Ptu", designLoads.Ptu, "lbf", ...
            "engine.designLoads - Ptu = FSU*FFU*PtL")];
else
    try
        ...
        inputs  = [ ...
            ...
            engine.eqInput("PpMax", preload.PpMax, "lbf", ...
                "engine.preload - NASA-STD-5020B Eq. 1, PpMax = PpiMax + Pth_max"), ...
            engine.eqInput("n", n, "", ...
                "Joint.LoadingPlaneFactor"), ...
            engine.eqInput("phi", phi, "", ...
                "engine.stiffness - NASA-STD-5020B Eq. 9, phi = kb/(kb + kc)"), ...
            engine.eqInput("P'tu", Pprime, "lbf", ...
                "NASA-STD-5020B Eq. 10 - P'tu = (Ptu_allow - PpMax)/(n*phi)"), ...
            engine.eqInput("Ptu", designLoads.Ptu, "lbf", ...
                "engine.designLoads - Ptu = FSU*FFU*PtL")];
    catch stiffErr
