classdef AppState < handle
    ...
    %
    %   Events (the ten in GUI_SPEC.md Section 5, deliberately coarse —
    %   never per-field):
    ...
    properties
        ...
        % Case-SCOPED — saved inside the case file (GUI_SPEC.md Section 15).
    ...
    methods
        function obj = AppState()
            ...
            obj.applyCaseStruct(gui.AppState.blankCaseState());
        end
    end
    ...
    methods (Access = private)
        ...
        function m = serializeMapping(obj)
            ...
            rows = gui.AppState.normalizeMapping(obj.Mapping);
            n = numel(rows);
            m = cell(1, n);
            for i = 1:n
                m{i} = struct( ...
    ...
    methods (Static)
        function st = readCaseFile(file)
            ...
            if isfield(raw, 'format')
                if ~strcmp(string(raw.format), gui.AppState.CaseFormat)
                    error('gui:AppState:badFormat', ...
                        ['%s: unsupported case format "%s" ' ...
                         '(expected "%s").'], ...
                        file, string(raw.format), gui.AppState.CaseFormat);
                end
                ...
                if isfield(raw, 'project') && isstruct(raw.project)
                    st.Project = gui.AppState.mergeProject(raw.project);
                end
                if isfield(raw, 'settings') && isstruct(raw.settings)
                    st.Settings = gui.AppState.parseSettings(raw.settings);
                end
                if isfield(raw, 'library') && isstruct(raw.library) && ...
                        isfield(raw.library, 'joints')
                    st.JointLibrary = ...
                        gui.AppState.parseJointLibrary(raw.library.joints);
                end
                ...
                if isfield(raw, 'mapping') && isstruct(raw.mapping) && ...
                    ...
                    st.Mapping = ...
                        gui.AppState.parseMapping(raw.mapping.elements);
                end
                if isfield(raw, 'forces') && isstruct(raw.forces) && ...
                        isfield(raw.forces, 'elements')
                    st.Elements = gui.AppState.parseElements(raw.forces);
                end
        ...
        function writeCaseFile(container, file)
            ...
            fid = fopen(file, 'w');
            if fid < 0
                error('gui:AppState:cannotWrite', ...
    ...
    methods (Static)
        function st = blankCaseState()
            ...
            st = struct( ...
                'Project',      gui.AppState.defaultProject(), ...
                'Settings',     gui.AppState.defaultSettings(), ...
                'Joint',        model.Joint(), ...
        ...
        function m = normalizeMapping(raw)
            ...
            for i = 1:numel(raw)
                ...
                m(end + 1) = gui.AppState.mappingRow( ...
        ...
        function s = parseSettings(raw)
            ...
            s = gui.AppState.defaultSettings();
        ...
        function jl = parseJointLibrary(rawJoints)
            ...
            for i = 1:numel(rawJoints)
                ...
                if ~isstruct(e) || ~isfield(e, 'name') || ~isfield(e, 'joint')
                    error('gui:AppState:badJointEntry', ...
        ...
        function m = parseMapping(rawElements)
            ...
            for i = 1:numel(rawElements)
                ...
                if ~isstruct(e) || ~isfield(e, 'elementId') || ...
                        ~isfield(e, 'jointName')
                    error('gui:AppState:badMappingEntry', ...
        ...
        function st = parseElements(rawForces)
            ...
            if isfield(rawForces, 'loadCases') && ~isempty(rawForces.loadCases)
                ...
                for i = 1:numel(raw)
                    ...
                    st.Cases(end + 1) = gui.AppState.elementCase( ...
            ...
            for i = 1:numel(raw)
                ...
                if ~any(names == lc)
                    error('gui:AppState:orphanForceRow', ...
