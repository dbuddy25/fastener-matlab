classdef BoltSizingPage < gui.Page
    ...
    methods (Access = private)
        function buildInputs(obj, g)
            ...
            obj.FactorsLabel.FontColor = gui.palette('mutedText');
        ...
        function items = materialItems(obj, role)
            ...
            items = {gui.BoltSizingPage.BlankChoice};
    ...
    methods (Access = private)
        ...
        function k = chosen(~, dd)
            ...
            k = string(dd.Value);
            if k == gui.BoltSizingPage.BlankChoice
                k = "";
            end
        end
        ...
        function showResults(obj, T, keys, matKey)
            ...
            for i = 1:n
                rows(i, :) = {char(T.ThreadSize(i)), char(T.Spec(i)), T.At(i), ...
                    gui.BoltSizingPage.fmtMargin(T.MS_TensionUlt(i)), ...
                    gui.BoltSizingPage.fmtMargin(T.MS_TensionYield(i)), ...
                    gui.BoltSizingPage.fmtMargin(T.MS_Shear(i)), ...
            ...
            if ~isempty(pass)
                addStyle(obj.ResultTable, uistyle('BackgroundColor', ...
                    gui.palette('tablePassBg')), 'row', pass);
            end
            if ~isempty(fail)
                addStyle(obj.ResultTable, uistyle('BackgroundColor', ...
                    gui.palette('tableFailBg')), 'row', fail);
            end
