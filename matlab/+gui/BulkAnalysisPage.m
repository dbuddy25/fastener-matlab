classdef BulkAnalysisPage < gui.Page
    ...
    methods
        ...
        function build(obj, parent)
            ...
            obj.StyleStale = uistyle('FontColor', gui.palette('mutedText'));
            obj.StyleFail  = uistyle( ...
                'BackgroundColor', gui.palette('tableFailBg'), ...
                'FontColor',       gui.palette('statusFail'));
    ...
    methods (Access = private)
        ...
        function buildTiers(obj, parent, row)
            ...
            obj.SummaryTable = gui.BulkAnalysisPage.fillTable(t1);
            ...
            obj.ElementTab   = t3;
            obj.ElementTable = gui.BulkAnalysisPage.fillTable(t3);
    ...
    methods (Access = private)
        function renderBanner(obj)
            ...
            if isempty(obj.State.BulkTable)
                ...
                obj.Banner.BackgroundColor = gui.palette('bannerInfoBg');
                obj.Banner.FontColor       = gui.palette('bannerInfoFg');
            ...
            elseif obj.State.BulkStale
                ...
                obj.Banner.BackgroundColor = gui.palette('bannerWarnBg');
                obj.Banner.FontColor       = gui.palette('bannerWarnFg');
        ...
        function renderVerdict(obj)
            ...
            elseif strlength(obj.CancelNote) > 0
                ...
                obj.VerdictLabel.FontColor = gui.palette('statusPass');
            end
        end
        ...
        function renderTiers(obj)
            ...
            ratioM = gui.MarginView.isRatio(shown);
        ...
        function renderElementTier(obj, T, shown, ratioM)
            ...
            err  = obj.errorMask(T);
            [~, failM] = gui.MarginView.passFail(M, ratioM);
            ...
            for k = 1:numel(idx)
                ...
                for c = 1:numel(shown)
                    if err(r)
                        ...
                        d{k, 5 + c} = gui.MarginView.cellText( ...
            ...
            obj.ElementTable.ColumnName = [ ...
                ...
                gui.MarginView.headerText(shown), {'Error'}];
        ...
        function renderSummaryTier(obj, T, shown, ratioM)
            ...
            for j = 1:numel(joints)
                ...
                env = gui.MarginView.envelope(M(mask, :), ratioM);
                [~, envFail] = gui.MarginView.passFail(env, ratioM);
                hasErr = any(err(mask));
                ...
                for c = 1:numel(shown)
                    row{3 + c} = gui.MarginView.cellText( ...
            ...
            obj.SummaryTable.ColumnName = [{'Joint', 'Max Axial', 'Max Shear'}, ...
                gui.MarginView.headerText(shown), {'Driving Load Case'}];
        ...
        function renderCaseTier(obj, T, shown, ratioM)
            ...
            for j = 1:numel(joints)
                ...
                row{1} = char(joints(j));
                for c = 1:numel(shown)
                    row{1 + c} = gui.MarginView.cellText( ...
                        env(c), ratioM(c), obj.CapCheck.Value);
                end
                d(end + 1, :) = row; %#ok<AGROW>
            end
            obj.CaseTable.ColumnName = [{'Joint'}, ...
                gui.MarginView.headerText(shown)];
        ...
        function styleTable(obj, t, d, marginCols)
            ...
            try
                ...
                for c = marginCols
                    for r = 1:size(d, 1)
                        if gui.BulkAnalysisPage.readsAsFailure(d{r, c})
    ...
    methods (Access = private)
        function [core, supp] = marginGroups(obj)
            ...
            mv   = vars(iS + 1:iW - 1);
            isCore = ismember(mv, gui.BulkAnalysisPage.CoreChecks);
            core = mv(isCore);
        ...
        function f = groupFailure(obj, T, cols)
            ...
            [~, failM] = gui.MarginView.passFail(M, ...
                gui.MarginView.isRatio(cols));
            f = any(failM, 2);
        end
    ...
    methods (Access = private)
        function [problems, pages] = gateProblems(obj)
            ...
            m   = gui.AppState.normalizeMapping(obj.State.Mapping);
        ...
        function c = caseRecord(~, el, name)
            c = gui.AppState.elementCase(name);   % defaults 1 / false
            if isempty(el.Cases)
                return
            end
    ...
    methods (Access = private)
        ...
        function runElements(obj, elements, progress)
            ...
            n    = numel(elements);
            grp  = gui.BulkAnalysisPage.sliceKeys(elements);
            nGrp = max(grp);
        ...
        function p = progressDialog(obj, n)
            ...
            p = struct( ...
                'Update',    @(done, total) gui.BulkAnalysisPage.tick(d, done, total), ...
    ...
    methods (Access = private)
        ...
        function [joint, loadCase] = singleJointInputs(~, joint, e, elements)
            ...
            mask = gui.BulkAnalysisPage.patternMask(elements, e);
    ...
    methods (Static, Access = private)
        ...
        function grp = sliceKeys(elements)
            ...
            for k = 1:n
                keys(k) = string(elements(k).LoadCaseName) + sep + ...
                    gui.BulkAnalysisPage.patternKey(elements(k));
            end
        ...
        function mask = patternMask(elements, e)
            ...
            for i = 1:n
                mask(i) = gui.BulkAnalysisPage.patternKey(elements(i)) == key ...
    ...
    methods
        ...
        function runSilently(obj)
            ...
            obj.runElements(elements, gui.BulkAnalysisPage.nullProgress());
        end
