classdef DefinedJointsPage < gui.Page
    %DEFINEDJOINTSPAGE  The case's saved joints (GUI_SPEC.md Section 3).
    %
    ...
    %   across every case). GUI_SPEC.md Section 15 resolved the two pages'
    ...
    methods (Access = private)
        function buildSummary(obj, parent, row, col)
            ...
            rows = gui.DefinedJointsPage.SummaryRows;
        ...
        function t = summaryText(~, j)
            ...
            t = struct();
            t.Bolt      = gui.DefinedJointsPage.orDash(j.Bolt.Designation);
            t.BoltMat   = gui.DefinedJointsPage.orDash(j.BoltMaterial.Name);
            ...
            t.Grip = gui.DefinedJointsPage.dim(j.GripLength, "in");
            ...
            t.Member    = member;
            t.MemberMat = gui.DefinedJointsPage.orDash(m.Material.Name);

            t.Torque     = gui.DefinedJointsPage.dim(j.PreloadSpec.NominalTorque, "in-lbf");
            t.NutFactor  = gui.DefinedJointsPage.dim(j.PreloadSpec.NutFactor, "");
    ...
    methods
        ...
        function tf = hasUnsavedJointWork(obj)
            ...
            if ~isempty(here)
                lib = obj.State.JointLibrary;
                for i = 1:numel(lib)
                    saved = gui.DefinedJointsPage.serialized(lib(i).Joint);
