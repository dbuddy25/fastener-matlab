classdef ElementForcesPage < gui.Page
    %ELEMENTFORCESPAGE  Imported FE forces, one load case per sheet.
    %   Bulk step 3 of 4 (the rail owns the 1-4 numbering, GUI_SPEC.md
    %   Section 3).
    %
    ...
    methods
        ...
        function build(obj, parent)
            ...
            % second coloured bar of its own: GUI_SPEC.md Section 12 puts
    ...
    methods (Access = private)
        function renderSummary(obj)
            ...
            for i = 1:nc
                ...
                d(i, :) = [{char(gui.ElementForcesPage.displayName(c.Name)), ...
        ...
        function renderDetail(obj)
            ...
            if isempty(obj.rows()) || isempty(idx)
                ...
                obj.DetailHeader.FontColor = gui.palette('mutedText');
            ...
            obj.DetailHeader.Text = sprintf( ...
                ...
                gui.ElementForcesPage.displayName(c.Name), c.Scale, ...
                rev, numel(ids));
            obj.DetailHeader.FontColor = gui.palette('defaultText');
        end
        ...
        function renderCrossCheck(obj)
            ...
            switch sev
                case "error"
                    obj.CrossCheck.BackgroundColor = gui.palette('bannerErrorBg');
                    obj.CrossCheck.FontColor       = gui.palette('bannerErrorFg');
                    obj.CrossCheck.FontWeight      = 'bold';
                case "warn"
                    obj.CrossCheck.BackgroundColor = gui.palette('bannerWarnBg');
                    obj.CrossCheck.FontColor       = gui.palette('bannerWarnFg');
                    obj.CrossCheck.FontWeight      = 'bold';
                case "info"
                    obj.CrossCheck.BackgroundColor = gui.palette('bannerInfoBg');
                    obj.CrossCheck.FontColor       = gui.palette('bannerInfoFg');
                    obj.CrossCheck.FontWeight      = 'normal';
                otherwise   % "ok"
                    obj.CrossCheck.BackgroundColor = gui.palette('fieldBg');
                    obj.CrossCheck.FontColor       = gui.palette('mutedText');
                    obj.CrossCheck.FontWeight      = 'normal';
            end
        end
    end
        ...
        function [sev, lines] = crossCheckReport(obj)
            ...
            if ~isempty(miss) && any(covered)
                lines(end + 1) = sprintf(['%d mapped element(s) have no ' ...
                    ...
                    gui.ElementForcesPage.idList(miss));
            end
            ...
            if ~isempty(extra)
                lines(end + 1) = sprintf(['%d element(s) in the forces ' ...
                    ...
                    numel(extra), gui.ElementForcesPage.idList(extra));
            end
            for i = 1:numel(cases)
                ...
                if ~isempty(cmiss)
                    lines(end + 1) = sprintf(['load case "%s": missing ' ...
                        ...
                        gui.ElementForcesPage.displayName(cases(i).Name), ...
                        numel(cmiss), ...
                        gui.ElementForcesPage.idList(cmiss)); %#ok<AGROW>
                end
            end
        ...
        function onSummaryEdited(obj, evt)
            ...
            try
                ...
                if r < 1 || r > numel(st.Cases)
                    error('gui:ElementForcesPage:staleRow', ...
                ...
                switch c
                    case 2
                        ...
                        if ~isscalar(v) || ~isfinite(v)
                            error('gui:ElementForcesPage:badScale', ...
                    ...
                    case 3
                        st.Cases(r).Reversible = ...
                            gui.ElementForcesPage.isTicked(evt.NewData);
                    otherwise
                        error('gui:ElementForcesPage:readOnly', ...
    ...
    methods (Access = private)
        ...
        function applyImport(obj, el, info, notes, mode)
            ...
            for i = 1:numel(el)
                ...
                row = gui.AppState.elementRow(id, lc, el(i).Forces);
                if isempty(hit)
                ...
                if isempty(st.Cases) || ...
                    ...
                    st.Cases(end + 1) = gui.AppState.elementCase(lc); %#ok<AGROW>
                end
            end
        ...
        function onExportTemplate(obj)
            ...
            try
                gui.ElementForcesPage.writeTemplate(file);
            catch err
        ...
        function onClearAllAnswered(obj, evt, n)
            ...
            obj.SelectedCase = "";
            obj.commit(gui.AppState.emptyElements(), ...
    ...
    methods (Static, Access = private)
        function names = rangeColumnNames()
            c = gui.ElementForcesPage.Components;
            names = cell(1, 12);
            for i = 1:6
            ...
            for i = 1:numel(info.Sheets)
                ...
                if ~sh.Parsed
                    if any(strcmpi(sh.Name, ...
                            gui.ElementForcesPage.InstructionSheets))
    ...
    methods
        ...
        function importWorkbook(obj, file)
            ...
            obj.applyImport(el, info, ...
                gui.ElementForcesPage.triage(info), "Merge");
        end
