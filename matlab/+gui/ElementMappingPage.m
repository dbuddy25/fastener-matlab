classdef ElementMappingPage < gui.Page
    %ELEMENTMAPPINGPAGE  Which defined joint each FE element is analyzed as.
    %   Bulk step 2 of 4 (GUI_SPEC.md Section 3 numbers the bulk workflow
    ...
    methods (Access = private)
        ...
        function buildTable(obj, parent, row)
            ...
            %   itself — the one place in gui outside the funnel, and the
            ...
            t.Tooltip = ['One row per FE element. Joint Name is a ' ...
                ...
                'several separate patterns. It only affects joint-mode ' ...
                'slip. Tick Remove to delete a row. ' ...
        ...
        function buildAssignRow(obj, parent, row)
            ...
            f.Tooltip = ['Optional. Written to the selected rows only when ' ...
                ...
                'existing pattern IDs alone. Used only for joint-mode ' ...
                'slip (5020B Eq. 84): elements sharing a pattern ID, joint ' ...
                'and load case are summed into the joint totals.'];
    ...
    methods (Access = private)
            ...
            try
                ...
                if any(dupMask)
                    addStyle(obj.Table, obj.StyleDup, 'cell', ...
                        gui.ElementMappingPage.cellIndex(dupMask, 1));
                end
                if any(unknownOrBlank)
                    addStyle(obj.Table, obj.StyleUnknown, 'cell', ...
                        gui.ElementMappingPage.cellIndex(unknownOrBlank, 2));
                end
            catch
        ...
        function renderSummary(obj, rows, dupMask, unknownMask, blankMask)
            ...
            if n == 0
                ...
                obj.SummaryLabel.FontColor  = gui.palette('mutedText');
            ...
            if nUnknwn > 0 || nBlank > 0
                obj.SummaryLabel.FontColor  = gui.palette('statusFail');
                obj.SummaryLabel.FontWeight = 'bold';
            elseif nDup > 0
                obj.SummaryLabel.FontColor  = gui.palette('statusWarn');
                obj.SummaryLabel.FontWeight = 'bold';
            else
                txt = [txt '   —   no issues'];
                obj.SummaryLabel.FontColor  = gui.palette('mutedText');
        ...
        function renderWarnBar(obj, rows, unknownMask)
            ...
            elseif strcmp(key, char(obj.WarnDismissedKey))
                ...
                obj.WarnLabel.Text = sprintf('%d joint(s) not in library: %s', ...
                    numel(shown), ...
                    char(gui.ElementMappingPage.idListSuffix(shown, false)));
    ...
    methods (Access = private)
        function rows = rows(obj)
            ...
            rows = gui.AppState.normalizeMapping(obj.State.Mapping);
        end
    ...
    methods (Access = private)
        function onCellEdited(obj, evt)
            ...
            try
                ...
                c = evt.Indices(2);
                if r < 1 || r > numel(rows)
                    error('gui:ElementMappingPage:staleRow', ...
                ...
                switch c
                    ...
                    case 4
                        if gui.ElementMappingPage.isTicked(evt.NewData)
                        ...
                        return
                    otherwise
                        error('gui:ElementMappingPage:badColumn', ...
        ...
        function v = validatedId(~, evt, label)
            ...
            if strlength(raw) == 0
                error('gui:ElementMappingPage:blankId', ...
                    '%s cannot be blank.', label);
            end
            if ~isempty(regexp(char(raw), '[\s,]', 'once'))
                error('gui:ElementMappingPage:badId', ...
        ...
        function v = validatedPattern(~, evt)
            raw = strtrim(string(evt.NewData));
            if contains(raw, ",")
                error('gui:ElementMappingPage:badPattern', ...
        ...
        function askUnknownJoint(obj, name, continuation)
            ...
            if n == 1
                ...
                msg = sprintf(['%d joint name(s) are not in the ' ...
                    ...
                    gui.ElementMappingPage.idListSuffix(name, false));
            end
    ...
    methods (Access = private)
        ...
        function onClearAllAnswered(obj, evt)
            ...
            obj.WarnDismissedKey = "";
            obj.commit(gui.AppState.emptyMapping(), 'Mapping cleared.');
        end
    ...
    methods (Access = private)
        ...
        function onBulkAddCommit(obj, dlg, ta, dd)
            ...
            try
                res = gui.ElementMappingPage.parseBulkAddText( ...
        ...
        function commitPairs(obj, ids, names, errs, what, choice)
            ...
            for k = 1:numel(ids)
                ...
                if isempty(hit)
                    rows(end + 1) = gui.AppState.mappingRow(ids(k), nm); %#ok<AGROW>
    ...
    methods (Access = private)
        function onImportCsv(obj)
            ...
            try
                [ids, names, pats, errs] = ...
                    gui.ElementMappingPage.parseMappingCsv(file);
            catch err
        ...
        function applyCsv(obj, ids, names, pats, errs, mode)
            ...
            if mode == "Replace"
                obj.State.Mapping = gui.AppState.emptyMapping();
            end
            unknown = unique(names(arrayfun( ...
                @(n) isempty(obj.findJoint(n)), names)));
            if isempty(unknown)
        ...
        function commitCsv(obj, ids, names, pats, errs, choice)
            ...
            for k = 1:numel(ids)
                ...
                if isempty(hit)
                    rows(end + 1) = ...
                        gui.AppState.mappingRow(ids(k), nm, pats(k)); %#ok<AGROW>
                    known(end + 1) = ids(k); %#ok<AGROW>
                    nAdded = nAdded + 1;
                else
                    rows(hit).JointName = nm;
        ...
        function writeCsv(obj, file)
            ...
            if isempty(rows)
                ...
                for i = 1:numel(rows)
                    lines(end + 1) = strjoin([ ...
                        gui.ElementMappingPage.csvField(rows(i).ElementID), ...
                        gui.ElementMappingPage.csvField(rows(i).JointName), ...
                        gui.ElementMappingPage.csvField(rows(i).PatternId)], ...
                        ","); %#ok<AGROW>
                end
            end
            fid = fopen(file, 'w');
            if fid < 0
                error('gui:ElementMappingPage:cannotWrite', ...
    ...
    methods (Static, Access = private)
        function [ids, names, pats, errs] = parseMappingCsv(file)
            ...
            if ~isfile(file)
                error('gui:ElementMappingPage:fileNotFound', ...
            ...
            for n = 1:numel(txt)
                ...
                fields = gui.ElementMappingPage.splitCsvLine(line);
                if ~headerSeen && gui.ElementMappingPage.looksLikeHeader(fields)
                    headerSeen = true;
                    continue
                end
        ...
        function res = parseBulkAddText(txt)
            ...
            for i = 1:numel(lines)
                ...
                if ~gui.ElementMappingPage.allNumeric(rest)
                    isPair(i)   = true;
            ...
            if ~any(isPair)
                res.mode = "ids";
                [res.ids, res.errs] = ...
                    gui.ElementMappingPage.parseIdTokens(txt);
                return
            end
        ...
        function updateDetectLabel(ta, dd, det)
            ...
            try
                gui.ElementMappingPage.detectInto(ta, dd, det);
            catch
        ...
        function detectInto(ta, dd, det)
            ...
            switch res.mode
                ...
                case "ids"
                    ...
                    det.FontColor = gui.palette('defaultText');
                    dd.Enable = 'on';
                otherwise
    ...
    methods
        ...
        function res = detectPaste(~, txt)
            res = gui.ElementMappingPage.parseBulkAddText(txt);
        end

        function f = splitCsv(~, line)
            f = gui.ElementMappingPage.splitCsvLine(line);
        end
