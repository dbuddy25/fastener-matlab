classdef FastenerApp < handle
    ...
    %     - drive File > New / Open / Save / Save As through gui.AppState's
    ...
    %       and the view renders what comes back (GUI_SPEC.md Section 6).
    %     - reach into a page's widgets. Cross-page effect goes through
    %       AppState (Section 5).
    %
    %   ERROR HANDLING (GUI_SPEC.md Section 11): every file/engine call is
    ...
    properties (Constant, Access = private)
        ...
        % active/idle (GUI_SPEC.md Section 3).
    ...
    properties (SetAccess = private)
        ...
        % "= gui.AppState()" would hand every window the same state
    ...
    properties (Access = private)
        ...
        % The References window (gui.ReferencesView). Owns its own
        ...
        SectionView = gui.JointSectionView.empty
    end
    ...
    methods
        ...
        function showSection(app)
            ...
            %   not hidden.
            if isempty(app.SectionView) || ~isvalid(app.SectionView)
                app.SectionView = gui.JointSectionView(app.State);
            end
    ...
    methods
        ...
        function refreshSummary(app)
            ...
            app.SummaryLabel.Text = sprintf( ...
                ...
                gui.FastenerApp.settingNum(s, 'NominalTempC'), ...
                gui.FastenerApp.settingNum(s, 'HotTempC'), ...
                gui.FastenerApp.settingNum(s, 'ColdTempC'), degC);
        end
        ...
        function tintStatus(app, active)
            ...
            if active
                app.StatusBox.BackgroundColor = gui.palette('statusActiveBg');
                app.StatusLabel.FontColor     = gui.palette('statusActiveFg');
            else
                app.StatusBox.BackgroundColor = gui.palette('footerBg');
                app.StatusLabel.FontColor     = gui.palette('mutedText');
            end
        end
        ...
        function p = page(app, pageId)
            %PAGE  The gui.Page instance for an id. For tests and dialogs.
            idx = app.indexOf(pageId);
            if isempty(idx)
                error('gui:FastenerApp:unknownPage', ...
                    'No page with id "%s".', pageId);
    ...
    methods (Access = private)
        function buildLayout(app)
            ...
            root.ColumnWidth = {gui.FastenerApp.RailWidth, '1x'};
            ...
            footer = uipanel(root, 'BorderType', 'none', ...
                'BackgroundColor', gui.palette('footerBg'));
            footer.Layout.Row    = 3;
            ...
            app.SummaryLabel.FontColor     = gui.palette('footerFg');
        end
    ...
    methods (Access = private)
        function specs = pageSpecs(app)
            %PAGESPECS  The rail contents, in order (GUI_SPEC.md Section 3).
            ...
            specs = { ...
                ...
                "",             "", gui.FactorsPage(s); ...
                "",             "", gui.TempLoadsPage(s); ...
                "SINGLE JOINT", "", gui.BoltSizingPage(s); ...
                "",             "", gui.JointConfigPage(s); ...
                "",             "", gui.ResultsPage(s); ...
                "BULK",        "1", gui.DefinedJointsPage(s); ...
                "",            "2", gui.ElementMappingPage(s); ...
                "",            "3", gui.ElementForcesPage(s); ...
                "",            "4", gui.BulkAnalysisPage(s); ...
                "REFERENCE",    "", gui.HardwareLibraryPage(s)};
        end
        ...
        function buildRail(app)
            ...
            for i = 1:n
                if strlength(specs{i, 1}) > 0
                    heights{end + 1} = gui.FastenerApp.RailSectionH; %#ok<AGROW>
                end
                heights{end + 1} = gui.FastenerApp.RailRowH; %#ok<AGROW>
            ...
            for i = 1:n
                ...
                if strlength(section) > 0
                    ...
                    lbl.FontSize  = 10;
                    lbl.FontColor = gui.palette('navSectionFg');
                    lbl.FontWeight = 'bold';
                ...
                b.FontColor = gui.palette('navIdleFg');
                b.Layout.Row    = rows(i);
                b.Layout.Column = 1;
        ...
        function refreshRailGlyphs(app)
            ...
            for i = 1:numel(app.Pages)
                switch app.Pages(i).Page.railStatus()
                    case "stale"
                        app.Pages(i).Glyph.Text = gui.FastenerApp.GlyphStale;
                        app.Pages(i).Glyph.FontColor = gui.palette('navStaleFg');
                    case "loaded"
                        app.Pages(i).Glyph.Text = gui.FastenerApp.GlyphLoaded;
                        app.Pages(i).Glyph.FontColor = gui.palette('navLoadedFg');
                    otherwise
    ...
    methods (Access = private)
        function buildMenus(app)
            %BUILDMENUS  File / Help (GUI_SPEC.md Section 4).
        ...
        function onHelpReferences(app)
            ...
            if isempty(app.ReferencesView) || ~isvalid(app.ReferencesView)
                app.ReferencesView = gui.ReferencesView();
            end
    ...
    methods (Access = private)
        ...
        function loadPath(app, file)
            ...
            if ~isfile(file)
                uialert(app.Fig, sprintf( ...
                    'No longer on disk:\n%s', file), 'Open failed');
                gui.recentFiles('remove', file);
                app.rebuildRecentMenu();
                return
            end
            try
                st = gui.AppState.readCaseFile(file);
            catch err
        ...
        function saveToFile(app, file)
            ...
            try
                ...
                gui.AppState.writeCaseFile(container, file);
            catch err
        ...
        function onCloseRequest(app)
            ...
            try
                ...
                warning('gui:FastenerApp:closePromptFailed', ...
