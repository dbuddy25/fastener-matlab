classdef HardwareLibraryPage < gui.Page
    ...
    %   (GUI_SPEC.md §15), and each page states its scope in-page.
    %
    ...
    methods
        ...
        function build(obj, parent)
            ...
            for spec = gui.HardwareLibraryPage.sectionSpecs()
    ...
    methods (Access = private)
        ...
        function buildSection(obj, spec)
            ...
            c.FontColor  = gui.palette('mutedText');
    ...
    methods (Access = private)
        ...
        function [rows, total] = sectionRows(obj, spec)
            ...
            for i = 1:numel(list)
                rows(i, :) = gui.HardwareLibraryPage.entryRow( ...
                    list{i}, spec.Fields);
            end
        end
        ...
        function onRowSelected(obj, entityId, evt)
            ...
            spec = gui.HardwareLibraryPage.specFor(char(entityId));
            ...
            obj.DetailArea.FontColor = gui.palette('defaultText');
        end
    ...
    methods (Access = private)
        ...
        function showLoadWarnings(obj)
            ...
            obj.DetailArea.Value = [ ...
                ...
                cellstr(w)];
            obj.DetailArea.FontColor = gui.palette('statusWarn');
        end
        ...
        function spec = activeSpec(obj)
            ...
            specs = gui.HardwareLibraryPage.sectionSpecs();
            spec  = specs(1);
            if isempty(obj.TabGroup) || ~isvalid(obj.TabGroup)
                return
            end
        ...
        function e = libraryEntry(obj, entityId, key)
            ...
            error("gui:HardwareLibraryPage:entryNotFound", ...
    ...
    methods (Access = private)
        function openForm(obj, spec, seed)
            ...
            lb.Layout.Row = 1;  lb.Layout.Column = 1;
            lb.FontColor  = gui.palette('mutedText');
        ...
        function entry = readForm(obj, spec)
            ...
            if ~isempty(missing)
                error("gui:HardwareLibraryPage:missingRequired", ...
        ...
        function lib = addTo(~, lib, entityId, entry)
            switch entityId
                ...
                otherwise
                    error("gui:HardwareLibraryPage:badEntityType", ...
