classdef JointConfigPage < gui.Page
    ...
    methods
        ...
        function refresh(obj)
            ...
            obj.BoltLengthField.Value  = ...
                gui.JointConfigPage.lengthChoice(j.Bolt.Length);
            ...
            obj.MemberTypeDropDown.Value = ...
                gui.JointConfigPage.memberTypeLabel(m.Type);
    ...
    methods (Access = private)
        ...
        function buildFlangeGroup(obj, parent, row)
            ...
            tips  = { ...
                ...
                'Hole centre to free edge e, in. Required for every layer with a thickness; untick Tear-out to skip that check.', ...
                'Run the shear tear-out check on this layer.'};

            n = gui.JointConfigPage.MaxFlangeLayers;
            ...
            for i = 1:n
                ...
                obj.FlangeMaterial{i} = uidropdown(fg, 'Items', mats, ...
                    'Value', gui.JointConfigPage.BlankChoice, 'Tooltip', tips{3});
        ...
        function w = buildWasherGroup(obj, parent, row, titleText, withSameAsHead)
            ...
            w.Spec = obj.addDropdown(b, r, 'Washer spec', ...
                {gui.JointConfigPage.CustomChoice}, ...
        ...
        function buildMemberGroup(obj, parent, row)
            ...
            obj.MemberTypeDropDown = obj.addDropdown(b, 1, 'Type', ...
                gui.JointConfigPage.memberTypeItems(), ...
            ...
            obj.MemberTypeDropDown.Value = gui.JointConfigPage.memberTypeLabel( ...
            ...
            [obj.MemberRatedField, obj.MemberRatedLabel] = ...
                obj.addLabelledText(b, 6, 'Rated pull-out (lbf)', ...
                    ['Manufacturer-rated load for the threaded member. ' ...
                     'Nut: the spec-rated ultimate IS the nut''s ultimate ' ...
                     'allowable, replacing the computed thread-shear value ' ...
                     '(5020B Sec. 4.4.1); yield is still computed. Insert: ' ...
                     'the specified internal-thread allowable, checked on ' ...
                     'its own row. Blank or 0 means no rating is claimed.']);
        ...
        function buildLoadsGroup(obj, parent, row)
            ...
            obj.BoltBendingField = obj.addLabelledText(b, 4, ...
                ...
                ['Most-loaded bolt bending limit MOMENT, IN-LBF (not lbf). ' ...
                 'Feeds the NASA-STD-5020B Eq. 20/22 fbu term. A value ' ...
                 'entered here is ALWAYS used, whatever the shear-transfer ' ...
                 'condition says: 4.4.4 exempts bending caused by the SHEAR ' ...
                 'loading, not a moment from prying or eccentric tension. ' ...
                 'Leave blank if there is no bending to account for.']);
        ...
        function buildAssumptionsGroup(obj, parent, row)
            ...
            obj.ShearPlaneDropDown = obj.addDropdown(b, 1, 'Shear plane', ...
                gui.JointConfigPage.enumItems('model.ShearPlaneCondition'), ...
            ...
            obj.SlipModeDropDown = obj.addDropdown(b, 2, 'Slip mode', ...
                gui.JointConfigPage.enumItems('model.SlipMode'), ...
            ...
            obj.BoltAxisDropDown = obj.addDropdown(b, 4, 'Bolt axis', ...
                gui.JointConfigPage.enumItems('model.BoltAxis'), ...
            ...
            % says that and the items lead with the answer.
            %
            % It stays a three-way choice rather than a checkbox because
            % the middle state is not a shade of the other two: "I checked
            % the fit and the exemption holds" and "nobody looked" both
            % produce fbu = 0 and are completely different claims in a
            % review. A toggle can only record one of them, and whichever
            % it dropped would be silently asserted for every joint.
            [bendLabels, bendData] = gui.JointConfigPage.bendingChoices();
            obj.ShearTransferDropDown = obj.addDropdown(b, 6, ...
                'Bolt bending (4.4.4)', bendLabels, ...
                ['How the NASA-STD-5020B 4.4.4 bending exemption is ' ...
                 'RECORDED. It does not decide whether a moment is used: ' ...
                 'a bolt bending limit moment entered under Applied loads ' ...
                 'is always included, on every setting, because 4.4.4 ' ...
                 'exempts bending caused by the SHEAR loading and a ' ...
                 'supplied moment may come from elsewhere. With no moment: ' ...
                 'default is fbu = 0 with the exemption ASSUMED; Verified ' ...
                 'means you confirmed a close-tolerance or interference ' ...
                 'fit; Accounted for means shear crosses a gap or ' ...
                 'clearance, and the interaction check reports not ' ...
                 'evaluated until you supply a moment.']);
            % Items are what a user reads and picks; ItemsData carries the
            % enum member names, so .Value round-trips through
            % enumFromLabel and the refresh path unchanged.
            obj.ShearTransferDropDown.ItemsData = bendData;
            obj.ShearTransferDropDown.Value = ...
                char(string(model.ShearTransferCondition.NotDeclared));
        ...
        function buildActionsGroup(obj, parent, row)
            ...
            obj.RequiredLabel.FontColor     = gui.palette('statusWarn');
        ...
        function b = groupGrid(~, panel, rows)
            ...
            % [1 3].
            b.ColumnWidth = {'1x', gui.JointConfigPage.ValueWRight, 4};
        ...
        function [d, lb] = addDropdown(obj, g, row, labelText, items, tip) %#ok<INUSD>
            ...
            if any(strcmp(items, gui.JointConfigPage.BlankChoice))
                d.Value = gui.JointConfigPage.BlankChoice;
            end
        end
        ...
        function restoreFamilyTokens(obj, saved)
            gui.JointConfigPage.restoreToken(obj.NutSpecDropDown, saved.Nut);
            gui.JointConfigPage.restoreToken(obj.washerGroup('Head').Spec, saved.Head);
            gui.JointConfigPage.restoreToken(obj.washerGroup('Nut').Spec, saved.NutWasher);
        end
        ...
        function items = libraryItems(obj, which)
            ...
            items = {gui.JointConfigPage.BlankChoice};
    ...
    methods (Access = private)
        ...
        function joint = buildJoint(obj)
            ...
            joint.ShearPlane = gui.JointConfigPage.enumFromLabel( ...
                'model.ShearPlaneCondition', obj.ShearPlaneDropDown.Value);
            joint.SlipMode   = gui.JointConfigPage.enumFromLabel( ...
                'model.SlipMode', obj.SlipModeDropDown.Value);
            joint.BoltAxis   = gui.JointConfigPage.enumFromLabel( ...
                'model.BoltAxis', obj.BoltAxisDropDown.Value);
            joint.ShearTransferCondition = gui.JointConfigPage.enumFromLabel( ...
        ...
        function setWasherFieldsEnable(~, w, groupLive)
            ...
            hasSizes = ~strcmp(w.Size.Value, gui.JointConfigPage.SizeNA);
        ...
        function applyNutSpec(obj)
            ...
            if ~isNut || spec == gui.JointConfigPage.CustomChoice
                if ~isNut
                    obj.NutSpecDropDown.Value = gui.JointConfigPage.CustomChoice;
                end
        ...
        function applyWasherSpec(obj, which)
            ...
            if spec == gui.JointConfigPage.CustomChoice
            ...
            gui.JointConfigPage.setItemsAndData(w.Size, labels, tokens);
        ...
        function fillWasherFromSize(obj, which)
            ...
            if key == gui.JointConfigPage.SizeNA || ~obj.State.LibraryOK
                return
            end
            entry = [];
            try
        ...
        function releaseWasherSpec(obj, which, message)
            ...
            w = obj.washerGroup(which);
            w.Spec.Value = gui.JointConfigPage.CustomChoice;
            gui.JointConfigPage.setItemsAndData(w.Size, ...
                {gui.JointConfigPage.SizeNA}, {gui.JointConfigPage.SizeNA});
            w.Size.Value = gui.JointConfigPage.SizeNA;
        ...
        function populateSpecPickers(obj)
            ...
            custom = gui.JointConfigPage.CustomChoice;
            try
                [tok, lab] = obj.State.Library.nutSpecs();
                gui.JointConfigPage.setItemsAndData(obj.NutSpecDropDown, ...
            ...
            try
                ...
                for which = {'Head', 'Nut'}
                    w = obj.washerGroup(which{1});
                    gui.JointConfigPage.setItemsAndData(w.Spec, ...
        ...
        function c = enabledColor(~, tf)
            if tf
                c = gui.palette('defaultText');
            else
                c = gui.palette('mutedText');
            end
        end

        function t = selectedMemberType(obj)
            t = gui.JointConfigPage.memberTypeFromLabel( ...
                obj.MemberTypeDropDown.Value);
        end
        ...
        function onAnalyze(obj)
            ...
            obj.setStatus(sprintf('Analyzed "%s".', ...
                gui.JointConfigPage.orPlaceholder(obj.State.Joint.Name, ...
                                                   'untitled joint')));
        end
        ...
        function syncJointLoadVisibility(obj)
            ...
            isJoint = gui.JointConfigPage.enumFromLabel('model.SlipMode', ...
        ...
        function updateBoltLengthChoices(obj)
            ...
            kept = gui.JointConfigPage.lengthChoice( ...
        ...
        function updateBoltLengthLabel(obj)
            ...
            lines = { ...
                gui.JointConfigPage.lineOrDash('Grip (stack + washers): %.4f in', ...
                    r.GripLength, 'Grip (stack + washers): —'), ...
                gui.JointConfigPage.lineOrDash('Minimum bolt length: %.4f in', ...
    ...
    methods (Access = private)
        ...
        function trySelect(obj, dd, value)
            ...
            if ~isempty(want) && any(strcmp(dd.Items, want))
                ...
                dd.Value = gui.JointConfigPage.BlankChoice;
            end
        end
    end
    ...
    methods (Static, Access = private)
        ...
        function [labels, data] = bendingChoices()
            %BENDINGCHOICES  Plain-language items for the §4.4.4 determination.
            %   data = the enum member names (ItemsData, what round-trips);
            ...
            % interference fits and requires it otherwise. Each item names
            % the determination first and the physical reason second, so
            % the control reads as a sentence -- "Bolt bending: required -
            % clearance or gap".
            friendly = struct( ...
                'NotDeclared', ...
                    'Not determined', ...
                'CloseToleranceOrInterference', ...
                    'Exempt - close or interference fit', ...
                'ClearanceOrGapped', ...
                    'Required - clearance or gap');
            for i = 1:numel(data)
                if isfield(friendly, data{i})
                    labels{i} = friendly.(data{i});
                end
            end
        end
        ...
        function s = lengthChoice(v)
            ...
            if isnan(v)
                s = gui.JointConfigPage.BlankChoice;
            else
                s = sprintf('%g', v);
            end
        end
        ...
        function items = memberTypeItems()
            ...
            for i = 1:numel(members)
                items{i} = gui.JointConfigPage.memberTypeLabel(members(i));
            end
        end
        ...
        function t = memberTypeFromLabel(txt)
            ...
            members = enumeration('model.ThreadedMemberType');
            for i = 1:numel(members)
                if strcmp(gui.JointConfigPage.memberTypeLabel(members(i)), want)
                    t = members(i);
                    return
                end
            end
