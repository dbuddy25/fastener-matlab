classdef (Abstract) Page < handle
    %PAGE  Base class for every page in the left rail (GUI_SPEC.md Section 5).
    %   A page owns one card in the shell's content area. It holds a
    %   reference to the shared gui.AppState, reads and writes it directly,
    ...
    properties (Access = private)
        % Route to the shell's status bar, injected by gui.FastenerApp via
    ...
    methods (Access = protected, Sealed)
        function lb = addBanner(~, parent, row, cols, text)
            ...
            lb.BackgroundColor = gui.palette('bannerInfoBg');
            lb.FontColor       = gui.palette('bannerInfoFg');
        end
    ...
    methods (Access = protected)
        ...
        function bindEdit(obj, control, callback)
            ...
            state = obj.State;
            control.ValueChangedFcn = @(s, e) gui.Page.runEdit(state, callback, s, e);
        end
    end
    ...
    methods (Static, Access = private)
        function setGroupCollapsed(pg, hdr, host, titleText, collapsed)
            ...
            if collapsed
                pg.RowHeight = {gui.Page.GroupHeaderH, 1};
                glyph = char(9654);   % right-pointing triangle
            else
                pg.RowHeight = {gui.Page.GroupHeaderH, 'fit'};
