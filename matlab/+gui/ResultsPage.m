classdef ResultsPage < gui.Page
    %RESULTSPAGE  Render one engine.Result (GUI_SPEC.md Section 8).
    ...
    methods
        ...
        function build(obj, parent)
            ...
            obj.StaleBanner.BackgroundColor = gui.palette('bannerWarnBg');
            obj.StaleBanner.FontColor       = gui.palette('bannerWarnFg');
            ...
            obj.ScopeLabel.FontColor     = gui.palette('mutedText');
        ...
        function refresh(obj)
            ...
            if ~hasResult
                ...
                obj.VerdictLabel.FontColor = gui.palette('mutedText');
                obj.StaleBanner.Visible   = 'off';
            ...
            obj.setStatus(sprintf('Showing results for "%s".', ...
                gui.ResultsPage.orPlaceholder(r.JointName, 'untitled joint')));
        end
    end
    ...
    methods (Access = private)
        ...
        function T = displayedTable(obj)
            ...
            rows = [obj.tableMargins(), obj.marginsNamed(gui.ResultsPage.DecisionRow)];
            ...
            for i = 1:numel(rows)
                ...
                Value(i)  = string(gui.ResultsPage.formatValue(rows(i), false));
                Status(i) = gui.ResultsPage.statusText(rows(i).Status);
        ...
        function buildTable(obj, g, row)
            ...
            obj.EmptyLabel.FontColor     = gui.palette('mutedText');
    ...
    methods (Access = private)
        function renderTable(obj)
            ...
            for i = 1:numel(rows)
                ...
                data{i, 2} = gui.ResultsPage.formatValue(rows(i), capOn);
                data{i, 3} = gui.ResultsPage.statusText(rows(i).Status);
            end
            obj.Table.Data = data;
        ...
        function applyStyles(obj, rows)
            ...
            try
                ...
                if obj.State.ResultStale
                    ...
                    idx = gui.ResultsPage.cellIndex(1:numel(rows), 1:3);
                    addStyle(obj.Table, ...
                        uistyle('BackgroundColor', gui.palette('tableNaBg'), ...
                                'FontColor', gui.palette('mutedText')), ...
                        'cell', idx);
                    return
                end
                ...
                if ~isempty(pass)
                    addStyle(obj.Table, ...
                        uistyle('BackgroundColor', gui.palette('tablePassBg')), ...
                        'cell', gui.ResultsPage.cellIndex(pass, 3));
                end
                ...
                if ~isempty(fail)
                    addStyle(obj.Table, ...
                        uistyle('BackgroundColor', gui.palette('tableFailBg'), ...
                                'FontWeight', 'bold'), ...
                        'cell', gui.ResultsPage.cellIndex(fail, 1:3));
                end
                ...
                if ~isempty(notEval)
                    addStyle(obj.Table, ...
                        uistyle('BackgroundColor', gui.palette('tableNotEvalBg')), ...
                        'cell', gui.ResultsPage.cellIndex(notEval, 1:3));
                end
            catch
        ...
        function renderReadouts(obj)
            ...
            obj.fillReadout(obj.PreloadValues, ...
                gui.ResultsPage.PreloadRows(:, 1), pre, stale);
            obj.fillReadout(obj.DesignLoadValues, ...
                gui.ResultsPage.DesignLoadRows(:, 1), load, stale);
        end

        function fillReadout(~, labels, names, src, stale)
            %FILLREADOUT  One panel's value labels, in spec order.
            if stale
                col = gui.palette('mutedText');
            else
                col = gui.palette('defaultText');
            end
            for i = 1:numel(labels)
                labels(i).Text      = gui.ResultsPage.readoutValue(src, names(i));
        ...
        function lines = bendingLines(~, r)
            ...
            if bnd.Included
                lines{end+1} = sprintf( ...
                    ...
                    gui.ResultsPage.withThousands(bnd.Fbu), bnd.Basis);
                lines{end+1} = sprintf( ...
        ...
        function lines = allowableLines(~, r)
            ...
            lines{end+1} = sprintf('  Governing: %s, %s lbf', ...
                gui.ResultsPage.orPlaceholder(a.GoverningMode, 'unknown'), ...
                gui.ResultsPage.withThousands(a.PtuAllow));

            if isfield(a, 'Modes') && ~isempty(a.Modes)
                for i = 1:numel(a.Modes)
                    m = a.Modes(i);
                    if m.Assessed
                        val = gui.ResultsPage.withThousands(m.Allowable) + " lbf";
                    else
        ...
        function updateDetail(obj)
            ...
            lines = {sprintf('%s - %s', char(m.Name), ...
                             gui.ResultsPage.statusText(m.Status))};

            notEval = string(m.Status) == "NotEvaluated";
            if ~notEval
                lines{end+1} = sprintf('  %s', ...
                    gui.ResultsPage.formatValue(m, logical(obj.CapCheck.Value)));
            end
        ...
        function lines = inputLines(~, m)
            ...
            for i = 1:numel(in)
                lines{end+1} = sprintf('  %s = %s%s', ...
                    ...
                    char(gui.ResultsPage.inputValueText(in(i))), ...
                    char(gui.ResultsPage.inputSourceText(in(i))));  %#ok<AGROW>
            end
        end
    ...
    methods (Access = private)
        ...
        function rows = tableMargins(obj)
            ...
            rows = obj.marginsNamed(gui.ResultsPage.TableRows);
        end
        ...
        function t = scopeFooterText(~)
            ...
            t = sprintf(['SCOPE: all 15 computed checks are shown - %d ' ...
                ...
                numel(gui.ResultsPage.TableRows));
        end
    end
    ...
    methods (Static, Access = private)
        function s = formatValue(m, capOn)
            ...
            %
            %   The rendering itself lives in gui.MarginView, shared with
            ...
            if m.Name == "Interaction"
                s = gui.MarginView.rText(m.R);
                return
            end
            s = gui.MarginView.msText(m.MS, capOn);
        end
        ...
        function s = readoutValue(src, name)
            ...
            s = gui.ResultsPage.withThousands(v);
        end
        ...
        function s = inputValueText(term)
            ...
            if isnan(term.Value)
                s = string(gui.MarginView.NotEvaluated);
                return
            end
    ...
    methods
        ...
        function h = readoutLabel(obj, name)
            ...
            k = find(gui.ResultsPage.PreloadRows(:, 1) == name, 1);
            if ~isempty(k)
                h = obj.PreloadValues(k);
                return
            end
            k = find(gui.ResultsPage.DesignLoadRows(:, 1) == name, 1);
            if isempty(k)
                error('gui:ResultsPage:noSuchReadout', ...
