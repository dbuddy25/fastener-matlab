...
%   ok = gui.openExternal(file, fig) opens `file` in whatever the OS uses
...
try
    ...
    elseif ismac
        ...
        if status ~= 0
            error("gui:openExternal:failed", "open returned %d", status);
        end
    else
        status = system(sprintf('xdg-open "%s" &', file));
        if status ~= 0
            error("gui:openExternal:failed", "xdg-open returned %d", status);
        end
    end
    ok = true;
catch err
