...
%   files = gui.recentFiles()               most-recent-first, existing only
%   files = gui.recentFiles('add', path)    move/insert at the front
%   files = gui.recentFiles('remove', path) drop an entry
%   files = gui.recentFiles('clear')        forget everything
%   p     = gui.recentFiles('path')         where the list is stored
%
%   The 'path' action exists so tGuiShell can snapshot and restore the
...
switch action
    ...
    case "clear"
        ...
        writeList(stored);
    otherwise
        error('gui:recentFiles:badAction', ...
