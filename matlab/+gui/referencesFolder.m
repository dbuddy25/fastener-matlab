...
%   p = gui.referencesFolder()          read the stored folder ("" if unset)
%   p = gui.referencesFolder(newPath)   store it, and return what was stored
%   p = gui.referencesFolder(_, store)  use a specific preference file
%
...
%
%   Stored in prefdir(), per user, exactly like gui.recentFiles -- not in
...
if strlength(p) == 0 && usingRealStore
    here = fileparts(mfilename("fullpath"));           % .../matlab/+gui
