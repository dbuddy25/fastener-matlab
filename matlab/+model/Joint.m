classdef Joint
    ...
    properties
        ...
        FrustumAngle          (1,1) double {mustBePositive, mustBeLessThan(FrustumAngle, 90)} = 30  % conical-frustum half-angle (deg), (0, 90) exclusive
    end
    ...
    methods
        function obj = Joint(args)
            ...
            model.Joint.checkTemperatureOrder( ...
    ...
    methods (Static)
        function checkTemperatureOrder(minTemp, refTemp, maxTemp)
            %CHECKTEMPERATUREORDER  Enforce MinTemperature <= ReferenceTemperature
            %   <= MaxTemperature — the ONE place this invariant is checked.
            %   The constructor calls it on every new Joint; ANY code path
            %   that assigns these three temperatures onto an already-built
            %   Joint by direct property write (bypassing the constructor —
            %   e.g. engine.private.applyGlobalSettings, stamping a
            %   settings file's global temperatures onto a parsed joint
            %   library) MUST call this too, so the invariant holds no
            %   matter how the temperatures got there. NaN entries (not yet
            %   set) skip the check, same as the constructor always has.
            temps = [minTemp, refTemp, maxTemp];
            if ~any(isnan(temps)) && ...
                    ~(minTemp <= refTemp && refTemp <= maxTemp)
