classdef PreloadSpec
    ...
    %   inputs to the min/max preload equations (NASA-STD-5020B Eq. 3/4/5, with the
    %   nominal preload from torque per NASA-STD-5020B Eq. 24).
    %
    %   Torque control is specified as NOMINAL torque + fractional tolerance
    %   (the NASA-STD-5020B c-factor form): a spec of "470 ± 20 in-lb" is
    %   NominalTorque = 470, TorqueTolerance = 20/470, giving the c-factors
    %   c_max = 1 + tol and c_min = 1 - tol of NASA-STD-5020B Eq. 3/4/5 (e.g. NASA-STD-5020B
    ...
    properties
        ...
        ThermalRate        (1,1) double = 0                              % lbf/°C; 0 (default) = compute from CTE/stiffness later. NOT an analyst input — no GUI control and no bulk-template column (removed, analysts found it confusing). Set programmatically only, by validation fixtures that need to reproduce a book answer key without full frustum geometry (e.g. validation.dabjSection9, 7.21 lbf/degF x 1.8 from DABJ Table 8-4).
    ...
    properties (Dependent)
        ...
        CMax               % NASA-STD-5020B c_max = 1 + TorqueTolerance (Eq. 3)
        CMin               % NASA-STD-5020B c_min = 1 - TorqueTolerance (Eq. 4/5)
    end
