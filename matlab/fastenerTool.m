function fastenerTool()
%FASTENERTOOL  Entry point for the MATLAB fastener analysis tool.
%
%   PHASE 1 STUB — proves the project is set up and runs. No analysis yet.
%   Later phases replace this with real launch/analysis behavior
%   (Phase 5.1 formalizes version/build stamping).
%
%   Usage (from the matlab/ folder, or with matlab/ on the path):
%       fastenerTool

    v = toolVersion();   % THE one definition -- see toolVersion.m
    fprintf("Fastener Analysis Tool (MATLAB) v%s\n", v);
    fprintf("  Stub entry point — engine not built yet (Phase 1).\n");
end
