classdef tGuiBulk < matlab.uitest.TestCase
    ...
    methods (Test)
        function theRailPageIsNoLongerAPlaceholder(testCase)
            testCase.verifyClass(testCase.Page, "gui.BulkAnalysisPage");
        end
    ...
    methods (Test)
        ...
        function gapsInBothDirectionsAreReportedNotSilentlyDropped(testCase)
            testCase.setUpCase();
            m = testCase.App.State.Mapping;
            m(end + 1) = gui.AppState.mappingRow("7777", testCase.jointName());
            testCase.App.State.Mapping = m;
    ...
    methods (Access = private)
        ...
        function setUpCase(testCase, opts)
            ...
            s.Factors      = c.Factors;
            s.Mapping      = gui.AppState.mappingRow("9001", c.Name, ...
                opts.PatternId);
            ...
            st.Rows(1)  = gui.AppState.elementRow("9001", "Liftoff", F);
            if opts.TwoCases
                st.Cases(2) = gui.AppState.elementCase("Landing");
                st.Rows(2)  = gui.AppState.elementRow("9001", "Landing", F);
            end
            s.Elements = st;
            s.clearDirty("");
        end
