classdef tGuiDefinedJoints < matlab.uitest.TestCase
    ...
    methods (Test)
        function theRailPageIsNoLongerAPlaceholder(testCase)
            testCase.verifyClass(testCase.Page, "gui.DefinedJointsPage");
        end
        ...
        function savedJointsRenderInStoredOrder(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryNamed(["Zeta", "Alpha", "Mid"]);
        ...
        function theCountIsSingularForExactlyOne(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryNamed("Only one");
            testCase.verifyEqual( ...
        ...
        function thePageFollowsStateItDidNotInitiate(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryNamed(["A", "B"]);
            testCase.assertEqual(numel(testCase.Page.listBox().Items), 2);

            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryNamed("A");
            testCase.verifyEqual( ...
    ...
    methods (Test)
        ...
        function selectingAJointShowsItsStoredValues(testCase)
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryWith("Bracket");
        ...
        function anUnsetValueReadsAsADashNotAZero(testCase)
            ...
            % specified.
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryWith("Bare", Configured = false);
            testCase.verifyEqual( ...
        ...
        function theGripLengthComesFromTheModelNotFromArithmeticHere(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryWith("Stacked");
        ...
        function theSummaryFollowsTheSelection(testCase)
            testCase.App.State.JointLibrary = [ ...
                tGuiDefinedJoints.libraryWith("First",  Torque = 470), ...
                tGuiDefinedJoints.libraryWith("Second", Torque = 250)];
            testCase.assertEqual(numel(testCase.Page.listBox().Items), 2);
        ...
        function aRefreshKeepsTheSelectedJointNotTheSelectedRow(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryNamed(["A", "B", "C"]);
    ...
    methods (Test)
        ...
        function aJointAlreadyInTheLibraryIsNotUnsavedWork(testCase)
            ...
            entry = tGuiDefinedJoints.libraryWith("Bracket", Torque = 310);
        ...
        function loadingCopiesTheJointOntoJointConfigAndGoesThere(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryWith("Bracket", Torque = 310);
        ...
        function renamingKeepsTheJointAndItsSelection(testCase)
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryWith("Old name", Torque = 310);
        ...
        function renamingCarriesTheElementMappingWithIt(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryWith("Old name");
            testCase.App.State.Mapping = [ ...
                gui.AppState.mappingRow("1001", "Old name"), ...
                gui.AppState.mappingRow("1002", "Elsewhere"), ...
                gui.AppState.mappingRow("1003", "Old name")];
        ...
        function aRenameOntoAnExistingNameIsRefused(testCase)
            ...
            testCase.App.State.JointLibrary = ...
                tGuiDefinedJoints.libraryNamed(["Alpha", "Beta"]);
