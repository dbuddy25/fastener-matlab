classdef tGuiElementForces < matlab.uitest.TestCase
    ...
    methods (Test)
        function theRailPageIsNoLongerAPlaceholder(testCase)
            testCase.verifyClass(testCase.Page, "gui.ElementForcesPage");
        end
    ...
    methods (Test)
        ...
        function aNegativeScaleIsRefused(testCase)
            ...
            % still be there — the same shape tGuiSetupPages uses for its
            % own input guards.
            testCase.loadForces();
            testCase.Page.editCell(1, 2, 2);
    ...
    methods (Test)
        ...
        function mergingUpdatesAMatchingElementAndLoadCase(testCase)
            ...
            tGuiElementForces.writeWorkbook(f1, {"Liftoff", [1001 1 2 3 0 0 0]});
            testCase.Page.importWorkbook(f1);

            f2 = testCase.tempFile(".xlsx");
            tGuiElementForces.writeWorkbook(f2, {"Liftoff", [1001 9 9 9 0 0 0]});
        ...
        function importingWhenForcesExistAsksFirst(testCase)
            testCase.loadForces();
            f = testCase.tempFile(".xlsx");
            tGuiElementForces.writeWorkbook(f, {"Later", [1009 1 2 3 0 0 0]});
    ...
    methods (Access = private)
        ...
        function setMapping(testCase, ids)
            ...
            for i = 1:numel(ids)
                m(i) = gui.AppState.mappingRow(ids(i), "Bracket");
            end
        ...
        function f = tempFile(testCase, ext)
            ...
            testCase.addTeardown(@() tGuiElementForces.deleteIfPresent(f));
        end
    end
    ...
    methods (Static, Access = private)
        function st = forcesState(caseNames, idsPerCase)
            ...
            for c = 1:numel(caseNames)
                st.Cases(c) = gui.AppState.elementCase(caseNames(c));
                ids = idsPerCase{c};
                for k = 1:numel(ids)
