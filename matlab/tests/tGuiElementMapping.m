classdef tGuiElementMapping < matlab.uitest.TestCase
    ...
    methods (Test)
        function theRailPageIsNoLongerAPlaceholder(testCase)
            testCase.verifyClass(testCase.Page, "gui.ElementMappingPage");
        end
    ...
    methods (Test)
        ...
        function aBlankJointNameIsItsOwnProblem(testCase)
            ...
            testCase.setLibrary("Bracket");
            testCase.App.State.Mapping = gui.AppState.mappingRow("1001", "");
    ...
    methods (Test)
        ...
        function editingThePatternIdAcceptsBlank(testCase)
            ...
            testCase.App.State.Mapping = ...
                gui.AppState.mappingRow("1001", "Bracket", "PLATE-1");
    ...
    methods (Test)
        ...
        function aJointNameWithACommaSurvivesTheCsvRoundTrip(testCase)
            ...
            testCase.App.State.Mapping = ...
                gui.AppState.mappingRow("1001", "Bracket, upper");
            f = testCase.tempFile(".csv");
        ...
        function csvRowsAreParsedIndependently(testCase)
            ...
            tGuiElementMapping.writeLines(f, [ ...
        ...
        function aHeaderlessCsvStillImports(testCase)
            f = testCase.tempFile(".csv");
            tGuiElementMapping.writeLines(f, ["1001,Bracket"; "1002,Bracket"]);
    ...
    methods (Test)
        ...
        function thePatternIdSurvivesTheCaseFile(testCase)
            ...
            gui.AppState.writeCaseFile(testCase.App.State.toCaseStruct(), f);
    ...
    methods (Access = private)
        ...
        function setMapping(testCase, ids, jointNames)
            ...
            for i = 1:numel(ids)
                m(i) = gui.AppState.mappingRow(ids(i), jointNames(i));
            end
            testCase.App.State.Mapping = m;
        end
        ...
        function f = tempFile(testCase, ext)
            ...
            testCase.addTeardown(@() tGuiElementMapping.deleteIfPresent(f));
        end
    end
    ...
    methods (Static, Access = private)
        function st = forcesWith(ids)
            ...
            st.Cases(1) = gui.AppState.elementCase("Liftoff");
            F = gui.AppState.zeroForces();
            for i = 1:numel(ids)
                st.Rows(i) = gui.AppState.elementRow( ...
                    string(ids(i)), "Liftoff", F);
            end
        end

        function writeLines(file, lines)
            tGuiElementMapping.writeRaw(file, ...
