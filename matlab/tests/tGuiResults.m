classdef tGuiResults < matlab.uitest.TestCase
    %TGUIRESULTS  Single Joint Results: the page that renders engine.Result.
    %
    ...
    methods (Test)
        ...
        function theVerdictNeverClaimsAPassWhileSomethingIsUnevaluated(testCase)
            ...
            testCase.showResult(tGuiResults.syntheticResult("noFailures"));
    ...
    methods (Test)
        ...
        function warningsRenderWithTheirSeverity(testCase)
            testCase.showResult(tGuiResults.syntheticResult("withWarning"));
            txt = strjoin(string(testCase.Page.warningArea().Value), newline);
    ...
    methods (Access = private)
        function showSynthetic(testCase)
            testCase.showResult(tGuiResults.syntheticResult("mixed"));
        end
    ...
    methods (Static, Access = private)
        ...
        function r = syntheticResult(variant)
            ...
            r = engine.Result( ...
                ...
                Margins   = margins, ...
                Narrative = tGuiResults.gluedDecision(), ...
                Warnings  = warnings);
            ...
            if gateAssured
                ...
                r.Gate = struct('Assessed', true, 'Assured', false, ...
                    ...
                    'Equation', "NASA-STD-5020B Eq. 7 (P'tu per Eq. 10)", ...
                    'Phi', 0.336, 'N', 1.00);
            end
    ...
    methods (Test)
        function theReadoutShowsThePreloadBandAndTheDesignLoads(testCase)
            testCase.showResult(tGuiResults.syntheticResult("withPreload"));
            p = testCase.Page;
        ...
        function aStaleReadoutIsMutedButKeepsItsNumbers(testCase)
            ...
            testCase.verifyEqual(vals(1).FontColor, gui.palette('mutedText'), ...
    ...
    methods (Test)
        ...
        function includedBendingReportsItsStressAndRatio(testCase)
            testCase.showResult(tGuiResults.syntheticResult("bendingIncluded"));
            txt = tGuiResults.bendingBlock(testCase);
        ...
        function aResultWithNoBendingBlockSaysSoRatherThanGuessing(testCase)
            ...
            r = tGuiResults.syntheticResult("mixed");
            r.Bending = struct();
            testCase.showResult(r);
            txt = tGuiResults.bendingBlock(testCase);
    ...
    methods (Test)
        ...
        function theGluedSentenceIsNotReprintedHere(testCase)
            ...
            txt = strjoin(string(p.detailArea().Value), newline);
            testCase.verifyFalse(contains(txt, tGuiResults.gluedDecision()), ...
